...
class Login:
    ...
    def login(self):
        if self.usuario.get() == "" or self.senha.get() == "":      #Adição:
        ...
        for u in usuarios:
            if u["usuario"] == self.usuario.get() and u["senha"] == self.senha.get(): 
                ...
                self.root.destroy() #self.root.withdraw() #Janela fica invisível e não fecha
                return
    ...
    def cadastrar(self):
        ...
        for u in usuarios:                                        #Verificação: se cadastro de usuário já existe.
            if u["usuario"] == self.usuario.get():                #Impedir gerar vários usuários iguais!
...
class SistemaClinica:
    ...
    def novo_paciente(self):
        ...
        def salvar():
            if campos["Nome Completo"].get() == "":             #Adição: Salvar paciente vazio
    ...
    def novo_atendimento(self):
        ...
        if not pacientes:                                                    #pacientes = carregar_dados(ARQ_PACIENTES)
            messagebox.showwarning("Aviso", "Cadastre um paciente primeiro") #nomes = [p["nome"] for p in pacientes]
            return                                                           #evitar salavr paciente vazio no combobox, sem atendimento
